...
Cada unidade aceita traz um fator `unitToCanonical` que multiplica o valor informado pelo aluno para obtê-lo na unidade de referência. A unidade canônica é a **única** entrada com `unitToCanonical === 1`; o campo `canonicalUnit` é derivado dessa entrada (ainda persistido no JSON para rótulos). Exemplo: `108 km/h × 0.2777777778 = 30 m/s`.
...
- Se `unitRequired: true`: exige `acceptedUnits` não vazio; cada unidade precisa de `unit` e `unitToCanonical` > 0; `aliases` é opcional (lista vazia permitida — o identificador `unit` já é aceito na resposta do aluno); deve existir **exatamente uma** unidade com fator `1` (canônica). O campo `canonicalUnit` é opcional na entrada (derivado na persistência); se enviado, deve coincidir com essa unidade.
