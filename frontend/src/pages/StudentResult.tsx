...
import { Award, AlertTriangle, XCircle, CheckCircle2, ArrowLeft, RefreshCw } from 'lucide-react';
...
export default function StudentResult({ submissionId }: { submissionId: string }) {
  ...
  const fetchResult = async (showRefreshIndicator = false) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(resData.message || 'Erro ao buscar resultado da submissão.');
      }
      setData(resData);
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-slate-400 text-sm">Buscando comprovante de envio...</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="max-w-md mx-auto text-center py-12 space-y-4">
        ...
        <p className="text-sm text-slate-400">{error || 'Não foi possível carregar os dados desta submissão.'}</p>
        <button
          onClick={() => navigateTo('/')}
      ...
      <div className="max-w-md mx-auto w-full space-y-6 py-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-mono uppercase mt-0.5">Status: Prova Aberta</p>
          </div>
        </div>
        ...
        <div className="glass-panel border border-cyan-500/20 bg-cyan-950/5 rounded-2xl p-6 text-center space-y-4">
          ...
          <div className="space-y-1">
            <h2 className="text-lg font-extrabold text-slate-200">Respostas Enviadas com Sucesso!</h2>
            <p className="text-xs text-slate-400 max-w-[280px] mx-auto">
              A nota e o gabarito detalhado estarão disponíveis assim que o professor encerrar a aplicação desta prova.
            </p>
          </div>
        </div>
        ...
        <div className="glass-panel border border-slate-800 rounded-2xl p-5 space-y-3.5 text-xs text-slate-400">
          <span className="text-[10px] uppercase font-bold text-slate-500 block tracking-wider text-center">Dados do Registro</span>
          <div className="space-y-2 font-mono bg-slate-950 p-3 rounded-xl border border-slate-900">
            <p><strong>Comprovante:</strong> <span className="text-cyan-400">{data.id}</span></p>
            <p><strong>Aluno:</strong> {data.student_name}</p>
            <p><strong>Matrícula:</strong> {data.student_identifier}</p>
            <p><strong>Data de Envio:</strong> {new Date(data.submitted_at).toLocaleString('pt-BR')}</p>
          </div>
  ...
  const maxPoints = data.answers ? data.answers.reduce((acc, curr) => acc + curr.points, 0) : 0;
    ...
    <div className="max-w-xl mx-auto w-full space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-500 font-mono uppercase mt-0.5">Prova Encerrada • Nota Divulgada</p>
        </div>
      </div>
      ...
      <div className="glass-panel border border-slate-800 rounded-2xl p-6 text-center relative overflow-hidden flex flex-col items-center">
        ...
        <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-4 block">Resultado Obtido</span>
        ...
        <div className="relative flex items-center justify-center w-28 h-28 rounded-full border-4 border-slate-850 mb-3 bg-slate-950">
          ...
          <div className="text-center">
            ...
            <span className="text-slate-500 text-xs block border-t border-slate-900 mt-0.5 pt-0.5">/ {maxPoints.toFixed(1)}</span>
          </div>
        </div>

        <h3 className="font-extrabold text-lg text-slate-200">{data.student_name}</h3>
        <p className="text-xs text-slate-400 font-mono">Matrícula: {data.student_identifier}</p>
        ...
        <div className="mt-4 flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900 border border-slate-850 text-xs text-slate-400 font-semibold">
          ...
          <span>Aproveitamento de <strong>{percentScore.toFixed(0)}%</strong></span>
        </div>
      </div>

      {/* Lista de Correção de Itens */}
      <div className="space-y-4">
        <h3 className="font-extrabold text-sm uppercase tracking-wider text-slate-400">Correção Questão por Questão</h3>
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-sm">
                    ...
                    {ans.subLabel && <span className="text-cyan-400 uppercase">{ans.subLabel}</span>}
                  </span>
                  <span className={`text-xs font-bold font-mono px-2 py-0.5 rounded ${
                    ans.isCorrect ? 'bg-emerald-950 text-emerald-400' : 'bg-rose-950/50 text-rose-400'
                  }`}>
                ...
                <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                  <div className="bg-slate-950/60 p-2 rounded-lg border border-slate-900/80">
                    <span className="text-[9px] uppercase font-bold text-slate-500 block">Sua Resposta</span>
                    <span className={`font-semibold ${ans.isCorrect ? 'text-emerald-400' : 'text-rose-450'}`}>
                      {ans.rawAnswer.trim() ? ans.rawAnswer : <em className="text-slate-700">Em Branco</em>}
                    </span>
                  </div>

                  <div className="bg-slate-950/60 p-2 rounded-lg border border-slate-900/80 flex flex-col justify-between">
                    <div>
                      <span className="text-[9px] uppercase font-bold text-slate-500 block">Correção</span>
