...
import { Plus, Trash2, ShieldAlert, Copy, Check, QrCode, ClipboardList, ArrowLeft } from 'lucide-react';
...
export default function TeacherCreate() {
  ...
  const handleAddItem = () => {
    const nextNum = items.length > 0 ? Math.max(...items.map(i => i.questionNumber)) + 1 : 1;
    setItems([
      ...items,
      {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const item of items) {
      if (item.accepted.length === 0) {
        setError(`A questão ${item.questionNumber}${item.subLabel ? item.subLabel : ''} precisa de pelo menos uma resposta correta.`);
        setLoading(false);
        return;
      }
      if (item.points <= 0) {
        setError(`A pontuação da questão ${item.questionNumber}${item.subLabel ? item.subLabel : ''} deve ser maior que zero.`);
        setLoading(false);
        return;
      }
    }
  ...
  const totalPoints = items.reduce((acc, curr) => acc + Number(curr.points || 0), 0);
    ...
    return (
      <div className="max-w-2xl mx-auto w-full space-y-6 animate-fade-in">
        <div className="glass-panel border border-emerald-500/20 bg-emerald-950/5 rounded-2xl p-6 text-center">
          ...
          <h2 className="text-2xl font-black text-emerald-400">Gabarito Criado com Sucesso!</h2>
          <p className="text-sm text-slate-400 mt-1">A prova foi publicada e está pronta para receber submissões.</p>
        </div>
          ...
          <div className="glass-panel border border-slate-800 rounded-2xl p-5 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-bold text-sm uppercase tracking-wider text-cyan-400">Acesso dos Alunos</h3>
              </div>
              <p className="text-xs text-slate-400 mb-4">
                Compartilhe o código ou o QR code abaixo para que os estudantes enviem suas respostas.
              </p>
            ...
            <div className="space-y-2">
              <div className="bg-slate-900 border border-slate-850 rounded-xl px-3 py-2 flex items-center justify-between">
                <div className="text-left">
                  <span className="text-[9px] uppercase font-bold text-slate-500 block">Código da Prova</span>
                  <span className="font-mono font-bold text-sm tracking-wider">{result.public_code}</span>
                </div>
                <button
                  ...
                  title="Copiar Código"
                >
                  {copiedPublic ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>

              <div className="bg-slate-900 border border-slate-850 rounded-xl px-3 py-2 flex items-center justify-between">
                <div className="text-left truncate mr-2">
                  <span className="text-[9px] uppercase font-bold text-slate-500 block">Link de Resposta</span>
                  <span className="font-mono text-xs text-slate-300 block truncate">{publicUrl}</span>
                </div>
                <button
          ...
          <div className="glass-panel border border-slate-800 rounded-2xl p-5 flex flex-col justify-between relative overflow-hidden">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-bold text-sm uppercase tracking-wider text-rose-400">Painel do Professor (Admin)</h3>
              </div>
              <p className="text-xs text-slate-400 mb-4">
                Use este token privado para acessar o painel de resultados e encerrar a prova posteriormente.
              </p>
              
              <div className="bg-rose-950/20 border border-rose-900/30 p-3.5 rounded-xl text-xs text-rose-300 mb-4 flex gap-2">
                <ShieldAlert className="w-5 h-5 shrink-0" />
                <span>
                  <strong>ATENÇÃO:</strong> Guarde este link administrativo! Por motivos de segurança, você não poderá consultá-lo novamente.
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="bg-slate-900 border border-slate-850 rounded-xl px-3 py-2 flex items-center justify-between">
                <div className="text-left truncate mr-2">
                  <span className="text-[9px] uppercase font-bold text-slate-500 block">Token Administrativo</span>
                  <span className="font-mono text-xs text-rose-400 block truncate font-bold">{result.admin_token}</span>
                </div>
                <button
                  ...
                  title="Copiar Token"
                >
                  {copiedAdmin ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>

              <div className="bg-slate-900 border border-slate-850 rounded-xl px-3 py-2 flex items-center justify-between">
                <div className="text-left truncate mr-2">
                  <span className="text-[9px] uppercase font-bold text-slate-500 block">Link do Painel Admin</span>
                  <span className="font-mono text-xs text-rose-300 block truncate">{adminUrl}</span>
                </div>
                <button
    ...
    <div className="max-w-2xl mx-auto w-full space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Defina o título, as questões e os gabaritos aceitos.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="glass-panel border border-slate-800 rounded-2xl p-5">
          <label htmlFor="examTitle" className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">
            Título da Prova
          </label>
          <input
            id="examTitle"
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-sm uppercase tracking-wider text-slate-400">Estrutura de Itens</h3>
            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-slate-900 border border-slate-800 text-slate-300">
              Total de pontos: <strong className="text-blue-400">{totalPoints.toFixed(1)}</strong>
            </span>
          </div>
            ...
            >
              ...
              <div className="grid grid-cols-12 gap-3 items-end">
                ...
                <div className="col-span-3">
                  <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Questão</label>
                  <input
                    type="number"
                    min="1"
                    value={item.questionNumber}
                    onChange={(e) => handleUpdateItem(item.id, { questionNumber: parseInt(e.target.value) || 1 })}
                ...
                {/* Subitem (ex: a, b) */}
                <div className="col-span-3">
                  <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Subitem</label>
                  <input
                    type="text"
                    maxLength={3}
                    placeholder="Sem"
                    value={item.subLabel}
                    onChange={(e) => handleUpdateItem(item.id, { subLabel: e.target.value.toLowerCase().replace(/[^a-z]/g, '') })}
                ...
                {/* Pontos */}
                <div className="col-span-3">
                  <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Pontos</label>
                  <input
                    type="number"
                    step="0.1"
                    min="0.1"
                    value={item.points}
                    onChange={(e) => handleUpdateItem(item.id, { points: parseFloat(e.target.value) || 0 })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-900">
                {/* Tipo de Questão */}
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase">Tipo de Resposta</label>
                  <select
                    value={item.answerType}
                    onChange={(e) => handleUpdateItem(item.id, { answerType: e.target.value as any })}
                ...
                {/* Gabarito da Questão */}
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase">Gabarito Oficial</label>
                    ...
                    <div className="flex gap-1.5 justify-between">
                      {['A', 'B', 'C', 'D', 'E'].map(option => (
                        <button
                          key={option}
                          type="button"
                          onClick={() => handleUpdateItem(item.id, { accepted: [option] })}
                    ...
                    <div className="space-y-2">
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={(e) => handleUpdateItem(item.id, { tempVariant: e.target.value })}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault();
                      ...
                      <div className="flex flex-wrap gap-1">
                        {item.accepted.map((val, variantIdx) => (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => handleRemoveTextVariant(item.id, variantIdx)}
                        ...
                        {item.accepted.length === 0 && (
                          <span className="text-[10px] text-slate-650 italic">Insira variações de resposta (ex: "cinco", "5")</span>
                        )}
                      </div>
                    </div>
                  )}
        ...
        >
          {loading ? 'Criando Gabarito...' : 'Publicar Gabarito e Gerar QR Code'}
        </button>
      </form>
    </div>
  );
